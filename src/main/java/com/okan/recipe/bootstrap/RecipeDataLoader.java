package com.okan.recipe.bootstrap;
...
public class RecipeDataLoader implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private List<Recipe> getRecipes() {
        ...
        Optional<Category> dutchCategoryOptional = categoryRepository.findByDescription("Dutch");
        if (!dutchCategoryOptional.isPresent()) {
            throw new RuntimeException("Expected Category Not Found");
        }

        Category americanCategory = americanCategoryOptional.get();
        Category mexicanCategory = mexicanCategoryOptional.get();
        Category dutchCategory = dutchCategoryOptional.get();
        ...
        // Pindakaas met Jam boterham :)
        Recipe boterham = new Recipe();
        boterham.setDescription("Boterham met Pindakaas en Jam");
        boterham.setPrepTime(1);
        boterham.setCookTime(0);
        boterham.setServings(1);
        boterham.setDifficulty(Difficulty.EASY);
        boterham.setDirections("1. Pak 2 sneetjes boterham.\n" +
                "2. smeer 1 boterham met pindakaas.\n" +
                "3. smeer de andere boterham met Jam.\n" +
                "4. plaats beide boterhammen op elkaar (pindakaas tegen de jam kant)\n\n" +
                "eet smakelijk!");
        Notes boterhamNote = new Notes();
        boterhamNote.setRecipeNotes("Smakkelijk eten :)");

        boterham.setNotes(boterhamNote);
        boterham.getCategories().add(dutchCategory);
        boterhamNote.setRecipe(boterham);

        boterham.getIngredients().add(new Ingredient("Boterham", new BigDecimal(2), eachUom, boterham));
        boterham.getIngredients().add(new Ingredient("Pindakaas", new BigDecimal(1), eachUom, boterham));
        boterham.getIngredients().add(new Ingredient("Jam", new BigDecimal(1), eachUom, boterham));

        recipes.add(boterham);
